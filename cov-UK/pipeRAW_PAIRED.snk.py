...
#370 samples test
#snakemake --snakefile pipeRAW_notemp.snk.py -j 24 --config ifq=/home/mgrau/projects/covid/ferrets/data/UK/paired/ fa=src/NC_045512.fasta out=/workspace/datasets/sars_cov_2/cov-UK/out_notemp/ -np

#All paired July 2020, ~20k samples
#snakemake --snakefile pipeRAW_notemp.snk.py -j 48 --config ifq=/workspace/datasets/sars_cov_2/cov-UK/raw/paired/ fa=src/NC_045512.fasta out=/workspace/datasets/sars_cov_2/cov-UK/out_pairedJuly/
...
#PAIRED READS
SAMPLES, PAIR= glob_wildcards(IFQ+"/{sample}_{pair}.fastq.gz")
...
rule filter:
   input:
        faR1=expand(IFQ+"{{sample}}_{pair}.fastq.gz", pair=["R1"]),
        faR2=expand(IFQ+"{{sample}}_{pair}.fastq.gz", pair=["R2"])
   output:
        R1out=workspace+"qualtrim/{sample}.R1.paired.fastq",
        R2out=workspace+"qualtrim/{sample}.R2.paired.fastq"
   shell:"""
        cutadapt --minimum-length 20 --cut 5 --cut -5 -o {output.R1out} -p {output.R2out} {input.faR1} {input.faR2}
        """
...
rule mapping:
    ...
    shell:"""
        ...
        bwa mem -R "@RG\\tID:$rgid\\tPL:illumina\\tPU:$rgpu\\tSM:{input.R1out}" {input.refFasta} -t {params.threads} {input.R1out} {input.R2out} > {output.sam} 
