#1) Count observed syn mutations (with a specific AF) and simulate (10**6 times) weighted random mutations along the genome (with the observed' size).
#2) Calculate the mean number of mutations per chunk and perform a g-test. 

#how to run
#python intra_simulate.py input output chunk_size AFmin AFmax
#python intra_simulate.py listMutationsAnn.csv sim.csv 1000 0.02 0.5
...
def get_observed_mutation_count():
    ...
    df=df[df.AF>float(sys.argv[4])]
    df=df[df.AF<float(sys.argv[5])]
